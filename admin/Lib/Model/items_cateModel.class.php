...
class items_cateModel extends RelationModel
{
	...
	function get_list($id=0){
		...
		$list=array();
		$res=$items_cate_mod->order('ordid DESC')->where('pid='.$id)->select();
		foreach($res as $key=>$val){
                    $val['level']=0;
                    $list[]=$val;
                    //二级分类
                    $arr=$items_cate_mod->order('ordid DESC')->where('pid='.$val['id'])->select();
                    //三级分类
                    foreach($arr as $k2=>$v2){
                        ...
                        $res3=$arr[$k2]['items']=$items_cate_mod->order('ordid DESC')->where('pid='.$v2['id'])->select();
